...
18:00 核对已按北京时间写在程序里，改服务器时区主要是让日志时间好读。
...
**方式 A：等北京时间 18:00**  
程序会自动找「当天日期+发货快递单」：找到就转换；找不到只打告警日志，继续用上一份 JSON，不会编造数据。
...
| `data/shipments.json` | 查询唯一数据源，由 ingest / 启动 / 18:00 生成 | 自动覆盖 |
...
4. 不手动执行时：最晚在北京时间 18:00 前传好当天文件，交给自动核对。  
5. 偶尔下载 `data/` 做备份。

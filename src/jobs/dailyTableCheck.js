...
function checkTodayTableUpdate() {
  ...
  logger.info(
    ...
    "北京时间18:00核对：当日表格已更新并完成 JSON 转换"
  );
  return result;
}
  ...
  cron.schedule(
    ...
    () => {
      try {
        ...
        logger.error({ err }, "北京时间18:00核对应货表格失败");
      }
    },
    { timezone: TIMEZONE }
  );
  logger.info("已登记北京时间每天下午18:00的表格更新核对");
}

module.exports = { startDailyTableCheck, checkTodayTableUpdate };

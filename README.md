...
- 北京时间每天下午 16:00、18:00、20:00、22:00 核对当天表格是否已更新并重新转换 JSON
- 同一唛头只返回被查询单号对应发货日期的全部记录；唛头为「无唛头」时提示联系客服，并返回该单到货状态
...
北京时间每天下午 16:00、18:00、20:00、22:00 会核对「当天日期+发货快递单」文件是否存在：已更新则重新转 JSON；未找到则写告警日志，并继续使用上一份 JSON，不会编造数据。
...
│   ├── jobs/
│   │   └── dailyTableCheck.js   # 北京时间 16/18/20/22:00 核对当日表格
│   ├── handlers/
│   │   ├── commands.js          # 命令处理
│   │   ├── tracking.js          # 快递单号查询
│   │   ├── text.js              # 文本与菜单按钮
